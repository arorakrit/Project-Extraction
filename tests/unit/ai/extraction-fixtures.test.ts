...
const FIXTURES_DIR = join(process.cwd(), 'tests/ai-fixtures/extraction')
...
describe('extraction golden fixtures', () => {
  const fixtures = loadFixtures()

  if (fixtures.length === 0) {
    it.skip('no fixtures present — add one under tests/ai-fixtures/extraction/<name>/', () => {})
    return
  }
  ...
  it.each(fixtures)(
    ...
    ({ recorded, expected }) => {
      const toolUse = recorded.content.find(
        b => b.type === 'tool_use' && b.name === 'record_coffee_label',
      )
      expect(toolUse, 'recorded-response.json must contain a tool_use block').toBeDefined()
      const parsed = ExtractedCoffeeSchema.parse(toolUse!.input)
